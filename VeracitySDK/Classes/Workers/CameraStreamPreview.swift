...
public class CameraStreamPreview: UIView {
    ...
    private var requestingStartSession = false
    private var isConfiged = false
    private var isStartedSession = false
    public weak var delegate: CameraStreamDelegate?
    ...
    public func activateSession() {
        ...
        return
        #else
        requestingStartSession = true
        guard isConfiged, !isStartedSession else {
            return
        }
...
extension CameraStreamPreview: AVCapturePhotoCaptureDelegate {
    public func photoOutput(_ output: AVCapturePhotoOutput, didFinishProcessingPhoto photo: AVCapturePhoto, error: Error?) {
        self.deactivateSession()
        
        if let error = error {
            debugLog("Failed to capture jpg photo \(error)!")
            delegate?.didFailedToTakeImage(error)
            return
        }
        
        guard let pixelBuffer = photo.pixelBuffer else {
            debugLog("Failed to acquire pixelBuffer!")
            return
        }
        ...
        guard let videoOrientation = captureVideoPreviewLayer?.connection?.videoOrientation else {
            debugLog("Failed to acquire videoOrientation!")
            return
        }
        ...
        guard let cgImage = CIContext().createCGImage(ciImage, from: ciImage.extent) else {
            debugLog("Couldn't create image from ")
            return
        }
...
fileprivate extension CameraStreamPreview {
    func setupLayout() {
        DispatchQueue.main.async {
            ...
            do {
                ...
                debugLog("Capture session failed with error: \(error.localizedDescription)")
            }
            #endif
    ...
    public func setupOverlay(_ maskSize: CGSize = CGSize(width: 300, height: 400),
